...
if identifyexecutor():find("Fluxus") and HWID.Flux == "cracked" then
...
tween.Create = function(argstable)
	local tweenName = argstable["Name"] or "None"
	local setTweenPart = argstable["Part"] or ""
	local EasingStyle = argstable["EasingStyle"] or Enum.EasingStyle.Linear
	local EasingDirection = argstable["EasingDirection"] or Enum.EasingDirection.Out
	local RepeatCount = argstable["RepeatCount"] or 0
	local Reverses = argstable["Reverse"] or false
	local DelayTween = argstable["Delay"] or 0
	local CFramePosition = argstable["CFrame/Position"] or nil -- enter {CFrame = CFrame} or {Position = Vector3.new(Position)}
	local TweenSpeed = argstable["Speed"] or 1
	local TweenPlay = argstable["Play"] or false
	if not TweenPlay then
		TS:Create(setTweenPart, TweenInfo.new(TweenSpeed, EasingStyle, EasingDirection, RepeatCount, Reverses, DelayTween), CFramePosition):Play()
	else
		TS:Create(setTweenPart, TweenInfo.new(TweenSpeed, EasingStyle, EasingDirection, RepeatCount, Reverses, DelayTween), CFramePosition)
	end
end

tween.Play = function(obj)
	return obj:Play()
end

tween.Remove = function(obj)
	if obj then
		obj = nil
	end
	return obj
...
local AutoFarmSection = BlatantTab:AddSection("Auto Farm","left")
		...
		if AutoFarm then
			while AutoFarm do
				for _, v in pairs(game.Workspace.Enemies:GetChildren()) do
					if v.Name:find("Bandit") and playerLevel < 10 or (not playerLevel == 10) then
...
if identifyexecutor():find("Fluxus") and (not shared.Whitelist1) and (not Whitelist.Flux[HWID.Flux]) then
	selfplr:Kick("User is not whitelist.")
elseif identifyexecutor():find("Fluxus") and shared.Whitelist1 and (not Whitelist.Flux[readfile(".hwid")]) and (not Whitelist.Flux[HWID.Flux]) then
	selfplr:Kick("Are you trying to bypass?")
elseif (not identifyexecutor():find("Fluxus")) and shared.FrWhitelist then
	selfplr:Kick("Are you trying to bypass?")
elseif (not identifyexecutor():find("Fluxus")) and shared.FrWhitelist and (not Whitelist.Normal[HWID.Normal]) then
	selfplr:Kick("Are you trying to bypass?")
end
